# Ralph parallel worker iteration prompt

You are running **one iteration** of an autonomous loop as part of a **parallel worker pool**. The wrapper script (`ralph-parallel.sh`) re-invokes you fresh each time with no prior session. Do not try to do more than one issue per iteration. Do not ask the user questions — make the reasonable call and proceed.

Your current **worker branch** is the branch you are on (e.g. `ralph/w0`). This branch is **your private workspace** — other workers use different branches. Push to this branch only. Never push to `main`.
...
- **Working directory**: this worker's git worktree (e.g. `/Users/arjen/Documents/bimboware/notepad-worker-0/`).
- **GitHub repo**: `samantha-network4all-bot/slate`.
- **Your branch**: the current branch — push here, not to `main`.
...
```
gh issue edit <N> --add-label in-progress --repo samantha-network4all-bot/slate
```
...
**Files you must never modify**: `PRD.md`, `PROMPT.md`, `PROMPT-parallel.md`, `ralph.sh`, `ralph-openrouter.sh`, `ralph-parallel.sh`, `.gitignore`, `notepad.png`.
...
3. Comment on the issue:
   ...
   "
   ```
4. Remove the `in-progress` label:
   ```
   gh issue edit <N> --remove-label in-progress --repo samantha-network4all-bot/slate
   ```
5. Write `failed:<N>` to `.ralph-status`.
6. Print `[ralph] iter result: failed:<N>` and stop.

---

## Step 4 — Submit & close via PR (only if build/tests passed)

Your current branch is the **worker branch** (e.g. `ralph/w0`). Do **NOT** push to `main` directly.
...
4. Push the worker branch:
   ...
   ```
   **If `git push` fails** (e.g. non-fast-forward rejection): do NOT force-push. Comment on the issue with the push error, write `failed:<N>` to `.ralph-status`, and stop.
...
6. Squash-merge the PR:
   ...
   ```
   **If merge fails** (conflict, checks failing): comment on the issue with the merge error, write `failed:<N>` to `.ralph-status`, and stop. Do NOT force-merge.
7. **Post a detailed completion comment, then close.** A bare "closed" comment is not acceptable — the user reads these comments to learn what shipped without opening the diff. Build the comment body in a temp file first (so multi-line markdown survives shell quoting), then post + close:
   ...
   ```markdown
   **Implemented in `<short SHA>`** via automated Ralph loop (parallel worker) · model: `<provider>/<model>`
...
8. Remove the `in-progress` label:
   ```
   gh issue edit <N> --remove-label in-progress --repo samantha-network4all-bot/slate
   ```
9. Write `closed:<N>` to `.ralph-status`.
10. Print `[ralph] iter result: closed:<N>` and stop.
...
- **Never** push to `main` — push only to the worker branch.
- **Never** remove the `in-progress` label from an issue you did not pick.
- **Never** modify `PRD.md`, `PROMPT.md`, `PROMPT-parallel.md`, `ralph.sh`, `ralph-openrouter.sh`, `ralph-parallel.sh`, `.gitignore`, `notepad.png`.
